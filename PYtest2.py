...
def main():
    ...
    BASICFONT = pygame.font.Font('freesannsbold.ttf',BASICFONTSIZE)
        ...
        for event in pygame.event.get():
            if event.type == MOUSEBUTTONUP:
                ...
                if(spotx,spoty) == (None,None):
                    ...
                    elif NEW_SURF.collidepoint(event.pos):
                        mainBoard,solutionSeq = generateNewPuzzle(80)
                        allMoves = []
...
def getLefTopOfTile(tileX,tileY):
...
def getSpotCLicked(board,x,y):
    for tileX in range(len(board)):
        for tileY in range(len(board[0])):
            left,top = getLefTopOfTile(tileX,tileY)
            tileRect = pygame.Rect(left,top,TILESIZE,TILESIZE)
            if tileRect.collidepoint(tileX,tileY):
                return (tileX,tileY)
    return None
...
def drawBoard(board,message):
    ...
    if message:
        ...
        left,top = getLefTopOfTile(0,0)
        width = BOARDWIDTH*TILESIZE
        height = BOARDHEIGHT*TILESIZE
        pygame.draw.rect(DISPLAYSURF,BORDERBOLOR,(left-5,top-5,width+11,height+11),4)
...
def slideAnimation(board,direction,message,animationSpeed):
    ...
    moveLeft,moveTop = getLefTopOfTile(movex,movey)
    ...
    for i in range(0,TILESIZE,animationSpeed):
        ...
        if direction == DOWN:
            drawTile(movex,movey,board[movex][movex],0,i)
        if direction == LEFT:
            drawTile(movex,movey,board[movex][movex],i,0)
        if direction == RIGHT:
            drawTile(movex,movey,board[movex][movex],-i,0)
...
def generateNewPuzzle(numSlides):
    ...
    for i in range(numSlides):
        ...
        slideAnimation(board,move,'new puzzle',int(TILESIZE/3))
        makeMove(board,move)
